...
import threading, random, time, re, subprocess
...
def connect(db_urls=None, *args, **kwargs):
    ...
    connection pool object. If `use_async` is False (the default),
    all database requests are made synchronously.

    @param db_url: A URL of the form C{psycopg://user:password@host/dbname}
    @type db_url: str

    @param use_async: if True, create an asynchronous connection pool object.
    @type use_async: bool
    ...
    for db_url in db_urls:
        ...
        if dsn['scheme'] not in ('psycopg2', 'psycopg'):
            raise RuntimeError("Only psycopg is supported for DB connections at this time.")

        none_keys = [k for k in dsn if dsn[k] is None]
    ...
    def runOperation(self, query, *args, **kwargs):
        ...
        while(pool):
            try:
                ...
                break
            except adbapi.ConnectionLost as e:
                if(pool == self.master):
                    raise e
                else:
                    log.msg("Expired slave %s during operation because of %s" % (pool.connkw['host'], str(e)))
                    try:
        ...
        while(pool):
            try:
                ...
                if(pool == self.master):
                    ...
                    log.msg("Expired slave %s during query because of %s" % (pool.connkw['host'], str(e)))
                    try:
                        self.slaves.remove(pool)
                        pool.close()
                    except:
...
class SynchronousConnectionPool(TimeoutConnectionPool):
    """
    This trivial subclass disables thread creation within the ConnectionPool
    object so that it may be used from within a synchronous application
    """
